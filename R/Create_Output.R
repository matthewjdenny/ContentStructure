...
#' @param model_output_directory The directory where all .Rdata files generated by Run_Full_Model() are stored. Defaults to NULL if we are not reading in any intermediate data from disk. 
  ...
  if(using_county_email_data){
    for(i in 1:length(data_list)){
      ...
      if(load_results_from_file){
        ...
        load( paste(model_output_directory,data_list[i],".Rdata", sep = ""))
        Estimation_Results <- NULL
      }else{
      ...
      temp <- Generate_Model_Diagnsotics(
                input_file = data_list[i] ,
                LS_Actor = 8, 
                input_folder_path = model_output_directory,
